...
public class WorldGuardHook
{
	...
	public WorldGuardHook(Trails plugin){
		...
		if(plugin.getConfig().getBoolean("Plugin-Integration.WorldGuard.decay-flag", true)) {
	...
	public boolean canCreateTrails(Player player, Location location) {
		...
		if (hasBypass(player, location) && plugin.getConfig().getBoolean("Plugin-Integration.WorldGuard.CheckBypass")) {
			return true;
		}else  {
	...
	public boolean canDecay(Location location) {
		if(!plugin.getConfig().getBoolean("Plugin-Integration.WorldGuard.decay-flag", true) || !plugin.getConfig().getBoolean("Plugin-Integration.WorldGuard.IntegrationEnabled", true)) return true;
		//return wg.canBuild(player, location);
		RegionQuery query = WorldGuard.getInstance().getPlatform().getRegionContainer().createQuery();
		com.sk89q.worldedit.util.Location loc = BukkitAdapter.adapt(location);
