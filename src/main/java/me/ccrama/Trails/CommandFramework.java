...
public class CommandFramework {
   ...
   public boolean handleCommand(CommandSender sender, String label, org.bukkit.command.Command cmd, String[] args) {
      for(int i = args.length; i >= 0; --i) {
         ...
         if(this.commandMap.containsKey(var12)) {
            ...
            CommandFramework.Command command = (CommandFramework.Command)((Method)entry.getKey()).getAnnotation(CommandFramework.Command.class);
            ...
            try {
               ((Method)entry.getKey()).invoke(entry.getValue(), new Object[]{new CommandFramework.CommandArgs(sender, cmd, label, args, var12.split("\\.").length - 1)});
      ...
      this.defaultCommand(new CommandFramework.CommandArgs(sender, cmd, label, args, 0));
      return true;
   }
   ...
   public void registerCommands(Object obj) {
      ...
      for(int var3 = 0; var3 < var4; ++var3) {
         ...
         int var9;
         String[] var10;
         if(m.getAnnotation(CommandFramework.Command.class) != null) {
            CommandFramework.Command var11 = (CommandFramework.Command)m.getAnnotation(CommandFramework.Command.class);
            if(m.getParameterTypes().length <= 1 && m.getParameterTypes()[0] == CommandFramework.CommandArgs.class) {
         ...
         } else if(m.getAnnotation(CommandFramework.Completer.class) != null) {
            CommandFramework.Completer comp = (CommandFramework.Completer)m.getAnnotation(CommandFramework.Completer.class);
            if(m.getParameterTypes().length <= 1 && m.getParameterTypes().length != 0 && m.getParameterTypes()[0] == CommandFramework.CommandArgs.class) {
   ...
   private void registerCommand(CommandFramework.Command command, String label, Method m, Object obj) {
      ...
      if(this.map.getCommand(cmdLabel) == null) {
         CommandFramework.BukkitCommand cmd = new CommandFramework.BukkitCommand(cmdLabel, this.plugin);
   ...
   private void registerCompleter(String label, Method m, Object obj) {
      String cmdLabel = label.replace(".", ",").split(",")[0].toLowerCase();
      CommandFramework.BukkitCommand ex;
      if(this.map.getCommand(cmdLabel) == null) {
         ex = new CommandFramework.BukkitCommand(cmdLabel, this.plugin);
         this.map.register(this.plugin.getName(), ex);
      }

      if(this.map.getCommand(cmdLabel) instanceof CommandFramework.BukkitCommand) {
         ex = (CommandFramework.BukkitCommand)this.map.getCommand(cmdLabel);
         if(ex.completer == null) {
            ex.completer = new CommandFramework.BukkitCompleter();
         }
         ...
         try {
            ...
            CommandFramework.BukkitCompleter completer;
            if(field.get(ex1) == null) {
               completer = new CommandFramework.BukkitCompleter();
               completer.addCompleter(label, m, obj);
               field.set(ex1, completer);
            } else if(field.get(ex1) instanceof CommandFramework.BukkitCompleter) {
               completer = (CommandFramework.BukkitCompleter)field.get(ex1);
               completer.addCompleter(label, m, obj);
            } else {
   ...
   private void defaultCommand(CommandFramework.CommandArgs args) {
   ...
   class BukkitCommand extends org.bukkit.command.Command {
      ...
      protected CommandFramework.BukkitCompleter completer;
	...
	public List<String> onTabComplete(CommandSender sender, org.bukkit.command.Command command, String label, String[] args) {
         for(int i = args.length; i >= 0; --i) {
            ...
            if(this.completers.containsKey(var11)) {
               ...
               try {
                  return (List<String>)((Method)entry.getKey()).invoke(entry.getValue(), new Object[]{CommandFramework.this.new CommandArgs(sender, command, label, args, var11.split("\\.").length - 1)});
